#!/bin/bash
# 运行 Z=64 LDM v10 列级正负样本损失隔离变量训练实验，不执行推理、消融或 CD。
...
case "${V10_VARIANT}" in
  ...
  D)
    ...
    ;;
  *)
    echo "Error: V10_VARIANT must be A, B, C, or D, got: ${V10_VARIANT}"
    exit 1
    ;;
esac
...
echo "v10 variant=${V10_VARIANT}, column pos/neg/temp=${MINI_LDM_COLUMN_POSITIVE_WEIGHT}/${MINI_LDM_COLUMN_NEGATIVE_WEIGHT}/${MINI_LDM_COLUMN_TEMPERATURE}"
audit_runtime_paths
