...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        输入:
            ...
            diffusion (KarrasDenoiser) - 扩散过程对象
            data (DataLoader) - 数据加载器
            batch_size (int) - 批次大小
            microbatch (int) - 微批次大小 (用于梯度累积)
            lr (float) - 学习率
            ema_rate (str/float) - EMA 衰减率 (可以是逗号分隔的字符串)
            log_interval (int) - 日志记录间隔步数
            save_interval (int) - 模型保存间隔步数
            resume_checkpoint (str) - 恢复训练的检查点路径
            use_fp16 (bool) - 是否使用混合精度训练
            fp16_scale_growth (float) - FP16 缩放增长率
            schedule_sampler (ScheduleSampler) - 时间步采样器
            weight_decay (float) - 权重衰减
            lr_anneal_steps (int) - 学习率退火步数
        输出:
            无
        作用: 初始化训练循环。
        逻辑:
        1. 保存各种配置参数。
        2. 初始化 MixedPrecisionTrainer。
        3. 初始化优化器 (RAdam)。
        4. 加载检查点或初始化 EMA 参数。
        """
        self.model = model
    ...
    def _load_and_sync_parameters(self):
        ...
        输出:
            无
        作用: 加载模型参数并同步到所有分布式进程。
        逻辑:
        1. 查找恢复检查点。
        2. 如果存在，加载参数。
        3. 使用 dist_util.sync_params 同步参数。
        """
    ...
    def forward_backward(self, batch, cond):
        ...
        输入:
            batch (Tensor) - 训练数据批次
            cond (Tensor/dict) - 条件数据
        输出:
            无
        作用: 前向传播计算损失并反向传播计算梯度。
        逻辑:
        1. 清零梯度。
        2. 将 batch 分割为 microbatch。
        3. 对每个 microbatch:
            a. 采样时间步 t。
            b. 计算损失 (diffusion.training_losses)。
            c. 反向传播。
        """
    ...
    def log_step(self):
        ...
        逻辑:
        1. 记录 step。
        2. 记录 samples (step * global_batch)。
        """
    ...
    def save(self):
        ...
        输出:
            无
        作用: 保存模型、EMA 参数和优化器状态。
        逻辑:
        1. 定义内部函数 save_checkpoint。
        2. 保存所有 EMA 版本的模型。
        3. 保存优化器状态。
        4. 保存当前主模型参数。
        """
...
class CMTrainLoop(TrainLoop):
    def __init__(
        ...
        **kwargs,
        ...
        """
        输入:
            target_model (nn.Module) - 目标模型 (用于一致性蒸馏)
            teacher_model (nn.Module) - 教师模型 (用于蒸馏)
            teacher_diffusion (KarrasDenoiser) - 教师模型的扩散器
            training_mode (str) - 训练模式 ('consistency_distillation', 'consistency_training', 'progdist')
            ema_scale_fn (function) - EMA 和 Scales 调度函数
            total_training_steps (int) - 总训练步数
            **kwargs - 传递给父类 TrainLoop 的参数
        输出:
            无
        作用: 初始化一致性模型训练循环。
        逻辑:
        1. 调用父类初始化。
        2. 加载并同步 target_model 和 teacher_model。
        3. 设置模型为 eval 模式 (target/teacher)。
        """
