...
def get_weightings(weight_schedule, snrs, sigma_data):
    """
    输入:
        weight_schedule: 权重调度策略 ("snr", "snr+1", "karras", "truncated-snr", "uniform")。
        snrs: 信噪比。
        sigma_data: 数据标准差。
    输出:
        weightings: 计算出的权重。
...
class KarrasDenoiser:
    def __init__(
        ...
        逻辑:
        1. 保存参数。
        2. 如果 loss_norm 为 "lpips"，初始化 LPIPS 损失函数。
        """
    ...
    def get_scalings(self, sigma):
        ...
        输出:
            ...
            c_out: 输出系数。
            c_in: 输入系数。
        作用: 计算 Karras 扩散模型的缩放系数。
        逻辑:
        根据 Karras 论文中的公式计算 c_skip, c_out, c_in。
        """
    ...
    def get_scalings_for_boundary_condition(self, sigma):
        ...
        输出:
            ...
            c_out: 输出系数。
            c_in: 输入系数。
        作用: 计算边界条件下的缩放系数 (用于一致性模型)。
        逻辑:
        根据一致性模型论文中的公式计算 c_skip, c_out, c_in，确保在 sigma_min 处的边界条件。
        """
    ...
    def training_losses(self, model, x_start, sigmas, model_kwargs=None, noise=None):
        ...
        作用: 计算训练损失。
        逻辑:
        1. 生成噪声并加到 x_start 上得到 x_t。
        2. 调用 denoise 方法得到去噪后的输出。
        3. 计算权重。
        4. 计算感知损失 (LPIPS) 和 MSE 损失。
        5. 组合损失。
        """
    ...
    def consistency_losses(
        ...
        输入:
            ...
            num_scales: 尺度数量。
            model_kwargs: 模型参数。
            target_model: 目标模型 (EMA 模型)。
            teacher_model: 教师模型 (可选)。
            teacher_diffusion: 教师扩散过程 (可选)。
            noise: 噪声 (可选)。
        输出:
            terms: 包含损失值的字典。
        作用: 计算一致性蒸馏损失。
        ...
        2. 采样时间步 t 和 t2。
        3. 生成噪声数据 x_t。
        4. 使用求解器 (Euler 或 Heun) 计算 x_t2。
        5. 计算当前模型的输出 distiller。
        6. 计算目标模型的输出 distiller_target。
        7. 计算损失 (L1, L2, LPIPS)。
        """
        if model_kwargs is None:
            model_kwargs = {}
    ...
    def progdist_losses(
        ...
        2. 采样时间步 t, t2, t3。
        3. 生成噪声数据 x_t。
        4. 计算当前模型的去噪结果 denoised_x。
        5. 使用 Euler 求解器从 x_t 步进到 x_t3。
        6. 计算目标 x (从 x_t3 反推)。
        7. 计算损失。
        """
        if model_kwargs is None:
...
def karras_sample(
    ...
    输入:
        ...
        sigma_max: 最大噪声标准差。
        rho: 噪声调度参数。
        sampler: 采样器类型 ("heun", "dpm", "ancestral", "onestep", "progdist", "euler", "multistep")。
        s_churn: 随机扰动参数。
        s_tmin: 随机扰动最小时间步。
        s_tmax: 随机扰动最大时间步。
        s_noise: 随机扰动噪声强度。
    ...
    elif prior_sample == True:
        if len(last_sample_result_list) >= prior_window_size:
            prior = sum(last_sample_result_list) / len(last_sample_result_list)

            noise = generator.randn(*shape, device=device)
            dims = prior.ndim
            sigmas_max_ind = schedule_sampler.get_max(device)
            prior_noised = prior + noise * sigma_max
    ...
    last_sample_result_list.append(x_0) 
...
def get_ancestral_step(sigma_from, sigma_to):
    ...
    输入:
        ...
        sigma_to: 目标噪声标准差。
    输出:
        sigma_down: 下一步的噪声标准差。
        sigma_up: 需要添加的噪声标准差。
    作用: 计算祖先采样的步长和噪声。
    逻辑:
    根据公式计算 sigma_down 和 sigma_up。
    """
    sigma_up = (
...
def sample_euler_ancestral(model, x, sigmas, generator, progress=False, callback=None):
    ...
    输出:
        x: 生成的样本。
    作用: 使用 Euler 方法进行祖先采样。
    逻辑:
    1. 遍历 sigmas。
    2. 计算去噪结果。
    3. 计算 sigma_down 和 sigma_up。
    4. 计算导数 d。
    5. 更新 x (Euler 步)。
    6. 添加噪声 (Langevin 步)。
    """
...
def sample_heun(
    ...
    输出:
        x: 生成的样本。
    作用: 使用 Heun 方法进行采样 (Karras et al. 2022 算法 2)。
    逻辑:
    1. 遍历 sigmas。
    2. 应用随机扰动 (churn)。
    3. 计算一阶导数 d。
