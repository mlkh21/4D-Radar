...
# ǿ�ƽ���ǰ��Ŀ��·�����ӵ� sys.path ����ǰ�棬�����ȼ��ص�ǰ��Ŀ��ģ��
...
def create_argparser():
    defaults = dict(
        # --- �������޸�Ĭ��ֵ ---
        teacher_model_path = "", # Ԥ���ʦģ��·�� (������Ԥѵ���õ�EDMģ��)
        schedule_sampler="uniform",
        lr=0.00005, # �޸�ѧϰ��
        weight_decay=0.0,
        lr_anneal_steps=0,
        global_batch_size=4, # Ԥ�� Batch Size (ע������ĵ��� global_batch_size)
        batch_size=-1,
        microbatch=-1, 
        ema_rate="0.999,0.9999,0.9999432189950708", # Ԥ�� EMA Rate
        log_interval=100,
        save_interval=20000,
        resume_checkpoint="",
        use_fp16=False, # Ԥ��ر� FP16
        fp16_scale_growth=1e-3,
        in_ch = 4, # Ԥ������ͨ�� 4 (Radar Voxel)
        out_ch = 4, # Ԥ�����ͨ�� 4 (Target Voxel)
        out_dir='./diffusion_consistency_radar/train_results/radar_cd', # Ԥ�����Ŀ¼
        dataset_dir = '/home/zxj/catkin_ws/src/4D-Radar-Diffusion/NTU4DRadLM_pre_processing/NTU4DRadLM_Pre', # ָ��Ԥ�����������
        dataloading_config = "./diffusion_consistency_radar/config/data_loading_config_train.yml", # ȷ��ָ����ȷ�������ļ�
        
        # --- �������� (���� cm_train_defaults �� model_and_diffusion_defaults) ---
        # ��Ҳ���������︲������Ĭ��ֵ�����磺
