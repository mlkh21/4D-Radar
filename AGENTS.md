...
## 主要研究内容
    ...
    1. 基于4D毫米波雷达点云的稀疏点云融合滤波方法研究；
    2. 基于4D毫米波雷达点云的稠密点云生成方法研究；

## 项目目标

作为 ../UVA-FusionMap项目中的子模块（感知层），完成4D毫米波雷达点云的稀疏点云融合滤波和稠密点云生成方法研究，形成可复现的代码和实验结果。
...
- 仅在哈希值参与协议身份绑定、父链校验、内容完整性校验或下游复现合同时计算和保存哈希；禁止为了重复留档、展示或无消费者的审计清单额外计算哈希。新增哈希前需明确其消费者和失败行为。
